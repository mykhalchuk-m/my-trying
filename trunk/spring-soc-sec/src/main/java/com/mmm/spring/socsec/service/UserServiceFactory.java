...
public class UserServiceFactory {
	...
	public static SocUserService getSocUserService(String provider,
			ConnectionRepository connectionRepository) {
		if (provider == null)
			throw new NullPointerException("unknown provider name");

		SocUserService sus = map.get(SocialSet.valueOf(provider));
		if (sus == null)
			throw new NullPointerException("connt get user service with name: "
					+ provider);
